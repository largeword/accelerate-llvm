...
import Data.Array.Accelerate hiding (scanr1)
import Data.Array.Accelerate.Debug.Trace
import qualified Prelude

scanr1 f xs = reverse $ scanl1 (Prelude.flip f) $ reverse xs
last xs = xs ! I1 i where i = case shape xs of I1 n -> n-1   
scanl'p xs = T2 ys y where
  xs' = scanl1 (+) xs
  y = unit $ last xs'
  ys = zipWith (-) xs' xs 
...
initialize points = T2 newPoints hull
  where
    ...
    T2 offsets counts = T2 (zip offsetsUpper offsetsLower) (zip countUpper countLower)
      where
        offsetsUpper,offsetsLower :: Acc (Vector Int)
        T2 offsetsUpper countUpper = scanl'p $ map f points
          where f p = cond (p == leftMost || p == rightMost) 0
                    $ cond (nonNormalizedDistance line p > 0.0) 1 0
        T2 offsetsLower countLower = scanl'p $ map f points
          where f p = cond (p == leftMost || p == rightMost) 0
                    $ cond (nonNormalizedDistance line p < 0.0) 1 0
    -- T2 offsets counts
...
step (T2 pointsWithLabels hull) = T2 newPoints newHull
  where
    ...
    T2 hullNewIndices hullNewSize =  scanl'p $ map (\p -> p == noPoint ? (1, 2)) furthestPerLabel
    ...
    T2 segmentOffsets newSize = scanl'p $ map (\(T2 size1 size2) -> size1 + size2) segmentSizes
